...
BusDataDT <- R6Class(
  ...
  private = list(
    ...
    read_from_db = function(db_con, query = NULL) { 
      if (is.null(query)) {
        ...
        arg_vec <- unlist(private$op_arg_lst[-length(private$op_arg_lst)])
    ...
    create_name = function() {
      ...
      for (arg in private$op_arg_lst[2:(length(private$op_arg_lst) - 1)]) {
  ...
  public = list(
    ...
    initialize = function(db_con, is_express = NULL, route = NULL, vehicle = NULL, direction = NULL, stop_gtfs_seq = NULL, query = NULL, opt_func = NULL) {
