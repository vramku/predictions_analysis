...
Pred_Data_Complt <- dbGetQuery(con, "SELECT vehicle, t_stamp, stop_gtfs_seq, hist_cum, rece_cum, sche_cum,
                                            t_predicted, t_measured
                                     FROM mta_bus_data")
dbDisconnect(con)
Pred_Data_Complt <- transform(Pred_Data_Complt, t_stamp = as_datetime(as.double(t_stamp), tz = timezone))
...
Pred_Data_Complt$pred_bin <- cut(Pred_Data_Complt$t_predicted, bin_cutoffs,  dig.lab = 5 )
...
Pred_Data_Analyzed <- as.data.table(plyr::ddply(Pred_Data_Complt, .(pred_bin), calc_new_pred, .parallel = T))
...
Pred_Data_ResMolt %>% ggplot(.) + 
  geom_bar(mapping = aes(x = pred_bin, fill = cut(.$value, res_cutoffs)), position = "dodge") + facet_grid(facet~.)